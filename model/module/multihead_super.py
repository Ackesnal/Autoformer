...
class RelativePosition2D_super(nn.Module):
    ...
    def __init__(self, num_units, max_relative_position):
        ...
        self.embeddings_table_v = nn.Parameter(torch.randn(max_relative_position * 2, num_units))
        self.embeddings_table_h = nn.Parameter(torch.randn(max_relative_position * 2, num_units))
        
        # self.embeddings_table = nn.Parameter(torch.randn(max_relative_position * 2, num_units))
        ...
        # self.sample_embeddings_table = None
    ...
    def set_sample_config(self, sample_head_dim):
        ...
        # self.sample_embeddings_table = self.embeddings_table[:,:sample_head_dim]
        
    def calc_sampled_param_num(self):
        return self.sample_embeddings_table_h.numel() + self.sample_embeddings_table_v.numel()
        # return self.sample_embeddings_table.numel()
        
    def forward(self, length_q, length_k, window_size = 13):
        ...
        # translate the distance from [1, 2 * max_relative_position], 0 is for the cls token
        ...
        # unfold_coords = nn.Unfold(kernel_size = window_size, padding = window_size//2)
        patch = int(length_q**0.5)
        coords_h = torch.arange(patch)
        coords_w = torch.arange(patch)
        coords = torch.stack(torch.meshgrid([coords_h, coords_w]))  # 2, Wh, Ww
        coords = torch.LongTensor(coords[0,:,:] - coords[1,:,:] + patch) #.type(torch.FloatTensor)
        # coords = coords.reshape(1,1,patch,patch)
        # coords = unfold_coords(coords).type(torch.LongTensor).squeeze()
        embeddings = self.sample_embeddings_table[coords] #.transpose(0,1).transpose(1,2)#.permute(1,2,0)
        """
        
        return embeddings

class AttentionSuper(nn.Module):
    def __init__(self, super_embed_dim, num_heads=8, window_size = 7, image_size=224, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., 
                 normalization = False, relative_position = False, num_patches = None, max_relative_position=14, scale=False, change_qkv = False):
        super().__init__()
        ...
        self.positions = torch.arange(0,image_size**2).reshape(1,1,image_size,image_size).type(torch.FloatTensor)
        self.filters = torch.zeros((image_size,image_size))
        

    def set_sample_config(self, sample_q_embed_dim=None, sample_num_heads=None, sample_in_embed_dim=None, sample_window_size=None):
        ...
        self.unfold = nn.Unfold(kernel_size = self.window_size, padding = self.window_size//2)
        self.filters = torch.zeros((self.image_size**2, self.image_size**2)).cuda()
    ...
    def forward(self, x, window_based = True):
        ...
        if window_based:
            ...
            positions = self.unfold(self.positions).squeeze().type(torch.LongTensor)
            for i in range(N):
                self.filters[i, positions[:,i]] = 1
            
            attn = attn * self.filters
            ...
            """
            unfold_k = nn.Unfold(kernel_size = self.window_size, padding = self.window_size//2)
            unfold_v = nn.Unfold(kernel_size = self.window_size, padding = self.window_size//2)
            
            k = k.reshape(B*self.sample_num_heads, int(N**(1/2)), int(N**(1/2)), -1).transpose(2,3).transpose(1,2)#.permute(0,3,1,2)
            k = unfold_k(k).transpose(1,2).reshape(B, self.sample_num_heads, N, self.window_size**2, C//self.sample_num_heads).transpose(-1,-2)
            v = v.reshape(B*self.sample_num_heads, int(N**(1/2)), int(N**(1/2)), -1).transpose(2,3).transpose(1,2)
            v = unfold_v(v).transpose(1,2).reshape(B, self.sample_num_heads, N, self.window_size**2, C//self.sample_num_heads)
            
            q = q.reshape(B, self.sample_num_heads, N, 1,  C//self.sample_num_heads)
            
            attn = torch.matmul(q, k) * self.sample_scale
            if self.relative_position:
                r_p_k = self.rel_pos_embed_k(N, N, self.window_size)
                attn = attn + torch.matmul(q, r_p_k) * self.sample_scale
            
            attn = attn.softmax(dim=-1)
            attn = self.attn_drop(attn)
            
            x = torch.matmul(attn, v).squeeze()
            if self.relative_position:
                r_p_v = self.rel_pos_embed_v(N, N, self.window_size)
                x = x + torch.matmul(attn, r_p_v.transpose(-1,-2)).squeeze()
