...
from model.module.embedding_super import PatchMerging
...
class Vision_TransformerSuper(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=2, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., window_size=7, qkv_bias=False, qk_scale=None, drop_rate=0., 
                 attn_drop_rate=0., drop_path_rate=0., pre_norm=True, scale=False, gp=False, 
                 relative_position=False, change_qkv=False, abs_pos = True, max_relative_position=7):
        super(Vision_TransformerSuper, self).__init__()
        # the configs of super arch
        self.super_embed_dim = embed_dim
        self.super_embed_dim_per_layer = []
        ...
        for i in range(depth):
            if i >= 2**stage + layer:
                self.downsamples.append(PatchMerging(input_resolution = [self.img_sizes[-1], self.img_sizes[-1]], dim=embed_dim))
                layer += 2**stage
                stage += 1
                embed_dim = embed_dim * 2
            self.super_embed_dim_per_layer.append(embed_dim) 
            self.img_sizes.append(img_size//(2**(stage-1)))
            
            self.spatial_blocks.append(TransformerEncoderLayer(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio,
                                                               window_size=window_size, image_size = self.img_sizes[-1],
            ...
            """
            if stage > 2:                                                   
                self.channel_blocks.append(TransformerEncoderLayer(dim=self.img_sizes[i]**2, num_heads=num_heads, 
                                                                   mlp_ratio=mlp_ratio, window_size=window_size,
                                                                   image_size = img_size,
                                                                   qkv_bias=qkv_bias, qk_scale=qk_scale, dropout=drop_rate,
                                                                   attn_drop=attn_drop_rate, drop_path=dpr[i],
                                                                   pre_norm=pre_norm, scale=self.scale,
                                                                   change_qkv=change_qkv, relative_position=False))
            """
    ...
    def set_sample_config(self, config: dict):
        ...
        self.sample_window_sizes = config['window_size']
        ...
        for i, downsample in enumerate(self.downsamples):
            downsample.set_sample_config(sample_embed_dim=self.sample_embed_dim[0]*(2**i))
        
        for i, spatial_blocks in enumerate(self.spatial_blocks):
            # not exceed sample layer number
            if i < self.sample_layer_num:
                sample_dropout = calc_dropout(self.super_dropout, self.sample_embed_dim[i], self.super_embed_dim_per_layer[i])
                sample_attn_dropout = calc_dropout(self.super_attn_dropout, self.sample_embed_dim[i], self.super_embed_dim_per_layer[i])
                spatial_blocks.set_sample_config(is_identity_layer=False,
                                                 sample_embed_dim=self.sample_embed_dim[i],
                                                 sample_mlp_ratio=self.sample_mlp_ratio[i],
                                                 sample_num_heads=self.sample_num_heads[i],
                                                 sample_window_size=self.sample_window_sizes[i],
                                                 sample_dropout=sample_dropout,
                                                 sample_out_dim=self.sample_output_dim[i],
                                                 sample_attn_dropout=sample_attn_dropout)
            # exceeds sample layer number
            else:
                spatial_blocks.set_sample_config(is_identity_layer=True)
        """        
        for i, channel_blocks in enumerate(self.channel_blocks):
            # not exceed sample layer number
            if i < self.sample_layer_num - 6:
                sample_dropout = calc_dropout(self.super_dropout, self.img_size, self.img_size)
                sample_attn_dropout = calc_dropout(self.super_attn_dropout, self.img_size, self.img_size)
                channel_blocks.set_sample_config(is_identity_layer=False,
                                                 sample_embed_dim=self.img_sizes[i+6] ** 2,
                                                 sample_mlp_ratio=self.sample_mlp_ratio[i+6],
                                                 sample_num_heads=self.sample_num_heads[i+6],
                                                 sample_window_size=self.sample_window_sizes[i+6],
                                                 sample_dropout=sample_dropout,
                                                 sample_out_dim=self.img_sizes[i+6] ** 2,
                                                 sample_attn_dropout=sample_attn_dropout)
            # exceeds sample layer number
            else:
                channel_blocks.set_sample_config(is_identity_layer=True)
        """        
        if self.pre_norm:
    ...
    def forward_features(self, x):
        ...
        for i in range(len(self.arch)):
            ...
            if stage > 2:
                if self.arch[i][0] == 0:
                    x = self.spatial_blocks[i](x)
                elif self.arch[i][0] == 1:
                    x = x.permute(0, 2, 1).contiguous()
                    x = self.channel_blocks[i-6](x, False)
                    x = x.permute(0, 2, 1).contiguous()
                if len(self.arch[i]) > 1 and self.arch[i][1] == 0:
                    x = self.spatial_blocks[i](x)
                elif len(self.arch[i]) > 1 and self.arch[i][1] == 1:
                    x = x.permute(0, 2, 1).contiguous()
                    x = self.channel_blocks[i-6](x, False)
                    x = x.permute(0, 2, 1).contiguous()
            else:
...
class Original_Vision_TransformerSuper(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        super(Original_Vision_TransformerSuper, self).__init__()
        # the configs of super arch
        self.super_embed_dim = embed_dim
        # self.super_embed_dim = args.embed_dim
        self.super_mlp_ratio = mlp_ratio
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., window_size=7, image_size=224, qkv_bias=False, qk_scale=None, 
                 dropout=0., attn_drop=0., drop_path=0., act_layer=nn.GELU, pre_norm=True, scale=False,
        ...
        self.attn = AttentionSuper(dim, num_heads=num_heads, window_size=window_size, image_size=image_size,
                                   qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop,
                                   proj_drop=dropout, scale=self.scale, 
                                   relative_position=self.relative_position, change_qkv=change_qkv,
                                   max_relative_position=max_relative_position)
    ...
    def set_sample_config(self, is_identity_layer, sample_embed_dim=None, sample_mlp_ratio=None, sample_num_heads=None, 
                          sample_window_size=None, sample_dropout=None, sample_attn_dropout=None, sample_out_dim=None):
        ...
        self.attn.set_sample_config(sample_q_embed_dim=self.sample_embed_dim, sample_num_heads=self.sample_num_heads_this_layer, 
                                    sample_in_embed_dim=self.sample_embed_dim, sample_window_size=self.sample_window_size)
    ...
    def forward(self, x, window_based = True):
        """
        Args:
